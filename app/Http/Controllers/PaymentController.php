...
class PaymentController extends Controller
{
    public function payByAlipay(Order $order, Request $request)
    {
        ...
        return app('alipay')->web([
            'out_trade_no' => $order->no, // 订单编号，需保证再商户端不重复
    ...
    // 服务器端回调
    public function alipayNotify($value='')
    {
        // 校验输入参数
    ...
    public function payByWechat(Order $order, Request $request)
    {
        ...
        $wechatOrder = app('wechat')->scan([
            ...
            'total_fee' => $order->total_amount * 100, // 与支付宝不通，微信支付的金额单位是分
            'body'  => '支付 Laravel Shop 的订单：' . $order->no, // 订单描述
        ]);
        // 把要转换的字符串作为QrCode 的构造函数参数
        $qrCode = new QrCode($wechatOrder->code_url);

        // 将生成的二维码图片数据以及字符串形式输出，并带上相遇的响应类型
